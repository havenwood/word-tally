...
pub(crate) struct Args {
    ...
    /// Delimiter between field and value (text format only).
    #[arg(
        short = 'd',
        long = "field-delimiter",
        value_name = "VALUE",
        help = "Delimiter between field and value [default: \" \"] (text format only)"
    )]
    field_delimiter: Option<String>,

    /// Delimiter between entries (text format only).
    #[arg(
        short = 'D',
        long,
        value_name = "VALUE",
        help = "Delimiter between entries [default: \"\\n\"] (text format only)"
    )]
